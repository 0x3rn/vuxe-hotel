        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {images.map((image) => (
            <div key={image.id} className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden group relative flex flex-col">
              <div className="relative aspect-square w-full">
                <Image src={image.imageUrl} alt="Social feed item" fill className="object-cover" />
              </div>
          ...
          {images.length === 0 && (
            <div className="col-span-full py-12 text-center text-gray-500 bg-white rounded-lg border border-gray-100">
              No social posts found. Add some!
            </div>
          )}
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
            <div className="flex justify-between items-center p-6 border-b border-gray-100">
              <h2 className="text-xl font-serif text-gray-900">{editingImage ? 'Edit Post' : 'Add New Post'}</h2>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              <div className="pt-4 flex justify-end gap-3 border-t border-gray-100 mt-6">
                ...
                <button type="submit" className="px-4 py-2 bg-primary text-primary-foreground rounded hover:bg-primary/90 transition-colors">Save Post</button>
              </div>
            </form>
          </div>
        </div>
